...
# Configure logging - Reuse existing configuration if run as part of app,
# or set up basic config if run standalone/tested separately.
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# --- Field Mapping (Frontend ID/JSON Key -> DB Column Name) ---
# Moved here as it defines the interface to the database table structure
FIELD_TO_COLUMN_MAP = {
...
def insert_model_card(cnxn: pyodbc.Connection, db_data: Dict[str, Any]):
    """
    Inserts a single model card record into the database.

    Args:
        cnxn: An active pyodbc connection object.
        db_data: A dictionary where keys are the actual database column names
                 (already mapped) and values are the data to be inserted.

    Raises:
        pyodbc.Error: If a database error occurs during insertion.
        ValueError: If db_data is empty.
    """
    if not db_data:
    ...
    try:
        ...
        # Ensure column names that are SQL keywords are bracketed (handled by map)
        ...
        # Consider logging values carefully in production due to potential sensitive data
        # logging.info(f"With values: {values}")

        cursor.execute(sql, values)
        # No commit here - let the calling function handle transaction commit/rollback

        logging.info("Successfully prepared insert for ModelCards table.")

    except pyodbc.Error as ex:
        logging.error(f"Database error during insert preparation/execution: {ex.args[0]} - {ex}")
...
# Example (can be removed or kept for direct testing)
if __name__ == '__main__':
    # This part would require db_connection to be importable
    # and assumes the table exists.
    print("This module provides data access functions.")
    # Example:
    # from db_connection import get_db_connection
    # test_conn = get_db_connection()
    # if test_conn:
    #     try:
    #         test_data = {"[name]": "DAL Test", "[description]": "Testing insert"}
    #         insert_model_card(test_conn, test_data)
    #         test_conn.commit() # Commit transaction
    #         print("Test insert successful (check DB).")
    #     except Exception as e:
    #         print(f"Test insert failed: {e}")
    #         test_conn.rollback() # Rollback on error
    #     finally:
    #         test_conn.close()
